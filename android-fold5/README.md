...
- 発走予定時刻と発走までの残り時間を表示
- 同一開催場の「前のレース」「次のレース」へワンタップ移動
- 開催一覧を15分間キャッシュし、再選択を高速化
...
5. レースNo.横の発走予定時刻を確認します。
6. 選手数、氏名、車番、競走得点、コメントを確認します。
7. 「オッズ非依存AIで予測」を押します。
8. 3連単上位30と選手別の1着・2着・3着・3着内確率を確認します。

選択中の「日付 → 開催場 → R」は画面に表示されます。レースNo.の
選択画面から開催場へ、開催場の選択画面から日付へ戻って選び直せます。
選択後は「前のレース」「次のレース」で、同じ開催場内だけを連続して
移動できます。「次のレース」は出走表取得までワンタップで開始します。
同じ日付の開催一覧は15分間キャッシュします。最新情報へ更新したい場合は
「開催一覧を最新情報に更新」を押してください。発走時刻が取得元に
明記されていない場合は推測表示しません。
