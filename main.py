from __future__ import annotations

import csv
from datetime import date
from html import escape
from io import StringIO

import streamlit as st
from current_race_snapshot import (
    save_current_race_snapshot,
)

from odds_browser import fetch_all_trifecta_odds_browser
from advanced_probability_model import calculate_expected_values_v2
from probability_model import (
    allocate_bet_budget,
    calculate_expected_values,
    extract_rider_numbers,
)
from racecard_browser import fetch_racecard_data_browser
from race_analysis import (
    build_bet_reasons,
    calculate_rider_indices,
)
from lineup_browser import fetch_lineup_browser
from lineup_from_comments import (
    infer_lineup_from_comments,
    merge_dom_order_with_comment_groups,
    select_authoritative_lineup,
)
from auto_score import build_automatic_scores
from race_catalog import fetch_race_catalog


st.set_page_config(
    page_title="競輪 妙味期待値アプリ",
    layout="wide",
)

st.title("競輪3連単 妙味期待値アプリ")
st.caption("Ver.1.13.0：自己評価の着順確率表示版")


def make_csv(
    rows: list[dict],
    fieldnames: list[str],
) -> bytes:
    buffer = StringIO()

    writer = csv.DictWriter(
        buffer,
        fieldnames=fieldnames,
        extrasaction="ignore",
    )

    writer.writeheader()

    for row in rows:
        writer.writerow(row)

    return buffer.getvalue().encode("utf-8-sig")


def render_odds_table(
    rows: list[dict],
) -> None:
    table_rows = []

    for row in rows:
        table_rows.append(
            f"""
            <tr>
                <td>{escape(str(row.get("人気", "")))}</td>
                <td>{escape(str(row.get("組番", "")))}</td>
                <td>{escape(str(row.get("オッズ", "")))}</td>
            </tr>
            """
        )

    html = f"""
    <div style="max-height:520px;overflow:auto;border:1px solid #ddd;">
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="position:sticky;top:0;background:#f5f5f5;">
                    <th style="padding:8px;border-bottom:1px solid #ddd;">人気</th>
                    <th style="padding:8px;border-bottom:1px solid #ddd;">組番</th>
                    <th style="padding:8px;border-bottom:1px solid #ddd;">オッズ</th>
                </tr>
            </thead>
            <tbody>
                {''.join(table_rows)}
            </tbody>
        </table>
    </div>
    """

    st.markdown(
        html,
        unsafe_allow_html=True,
    )


def render_expected_value_table(
    rows: list[dict],
    threshold: float,
) -> None:
    table_rows = []

    for index, row in enumerate(rows, start=1):
        expected_return = float(
            row.get("期待回収率", 0)
        )

        recommendation = (
            "候補"
            if expected_return >= threshold
            else ""
        )

        table_rows.append(
            f"""
            <tr>
                <td>{index}</td>
                <td>{escape(str(row["組番"]))}</td>
                <td>{row["人気"]}</td>
                <td>{row["オッズ"]:.1f}</td>
                <td>{row["モデル確率"] * 100:.3f}%</td>
                <td>{row["市場確率"] * 100:.3f}%</td>
                <td>{row["フェアオッズ"]:.1f}</td>
                <td>{row["期待回収率"]:.3f}</td>
                <td>{row["期待利益率"] * 100:+.1f}%</td>
                <td>{row["妙味倍率"]:.2f}</td>
                <td>{row["参考購入額"]:,}円</td>
                <td>{recommendation}</td>
            </tr>
            """
        )

    html = f"""
    <div style="max-height:700px;overflow:auto;border:1px solid #ddd;">
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
            <thead>
                <tr style="position:sticky;top:0;background:#f5f5f5;">
                    <th>順位</th>
                    <th>組番</th>
                    <th>人気</th>
                    <th>オッズ</th>
                    <th>モデル確率</th>
                    <th>市場確率</th>
                    <th>フェアオッズ</th>
                    <th>期待回収率</th>
                    <th>期待利益率</th>
                    <th>妙味倍率</th>
                    <th>参考購入額</th>
                    <th>判定</th>
                </tr>
            </thead>
            <tbody>
                {''.join(table_rows)}
            </tbody>
        </table>
    </div>
    """

    st.markdown(
        html,
        unsafe_allow_html=True,
    )


defaults = {
    "catalog": {},
    "catalog_logs": [],
    "odds": [],
    "odds_logs": [],
    "expected_values": [],
    "rider_data": [],
    "rider_logs": [],
    "automatic_scores": {},
    "lineup_groups": [],
    "lineup_logs": [],
    "bet_plan": [],
    "bet_plan_message": "",
    "bet_plan_executed": False,
    "bet_plan_candidate_count": 0,
    "plan_minimum_return": 1.00,
    "plan_budget": 10_000,
    "plan_maximum_bets": 10,
    "plan_maximum_ratio": 0.25,
    "plan_minimum_unit": 100,
    "current_collection_scope": "",
    "current_collection_message": "",
}

for key, default in defaults.items():
    if key not in st.session_state:
        st.session_state[key] = default


selected_date = st.date_input(
    "開催日",
    value=date.today(),
)

if st.button(
    "この日の開催一覧を取得",
    type="primary",
):
    try:
        with st.spinner(
            "WINTICKETから開催一覧を確認しています"
        ):
            catalog, logs = fetch_race_catalog(
                selected_date
            )

        st.session_state.catalog = catalog
        st.session_state.catalog_logs = logs
        st.session_state.odds = []
        st.session_state.odds_logs = []
        st.session_state.expected_values = []
        st.session_state.rider_data = []
        st.session_state.rider_logs = []
        st.session_state.automatic_scores = {}

    except Exception as exc:
        st.session_state.catalog = {}
        st.session_state.catalog_logs = [
            f"開催一覧取得エラー: "
            f"{type(exc).__name__}: {exc}"
        ]


catalog = st.session_state.catalog

if catalog:
    venue = st.selectbox(
        "競輪場",
        list(catalog.keys()),
    )

    races = catalog[venue]

    race_labels = [
        f"{race['race_number']}R"
        for race in races
    ]

    selected_label = st.selectbox(
        "レース番号",
        race_labels,
    )

    selected_race = races[
        race_labels.index(selected_label)
    ]

    st.success(
        f"選択中：{selected_date:%Y年%m月%d日}"
        f"・{venue}・{selected_label}"
    )

    st.code(
        selected_race["url"],
        language=None,
    )

    if st.button(
        "レースデータを取得",
        type="primary",
        help=(
            "オッズ・出走表・選手・並びを取得します。"
            "オッズ非表示時も独立AI用データの"
            "取得は継続します。"
        ),
    ):
        st.session_state.odds = []
        st.session_state.odds_logs = []
        st.session_state.rider_data = []
        st.session_state.rider_logs = []
        st.session_state.automatic_scores = {}
        st.session_state.lineup_groups = []
        st.session_state.lineup_logs = []
        st.session_state.expected_values = []
        st.session_state.current_collection_scope = ""
        st.session_state.current_collection_message = ""

        st.session_state.selected_date = (
            selected_date
        )
        st.session_state.selected_venue = venue
        st.session_state.selected_race_number = (
            int(selected_race["race_number"])
        )
        st.session_state.selected_race_url = str(
            selected_race["url"]
        )
        st.session_state.race_title = (
            f"{venue} {selected_label}"
        )

        try:
            with st.spinner(
                "全3連単オッズを取得しています"
            ):
                odds, logs = (
                    fetch_all_trifecta_odds_browser(
                        selected_race["url"]
                    )
                )

            st.session_state.odds_logs = logs
            odds_complete = any(
                str(log).strip()
                == "オッズデータ完全性: OK"
                for log in logs
            )
            st.session_state.odds = (
                odds
                if odds_complete
                else []
            )

        except Exception as exc:
            st.session_state.odds = []
            st.session_state.odds_logs = [
                "オッズ取得Workerで例外が"
                "発生しました。",
                f"{type(exc).__name__}: {exc}",
            ]

        if not st.session_state.odds:
            st.session_state.odds_logs.extend(
                [
                    (
                        "オッズ状態: 非表示または"
                        "完全データ未取得"
                    ),
                    (
                        "オッズ非依存AI用の"
                        "出走表・並び取得を継続します。"
                    ),
                ]
            )

        try:
            rider_data, rider_logs = (
                fetch_racecard_data_browser(
                    selected_race["url"]
                )
            )

            if not rider_data:
                raise RuntimeError(
                    "出走表・選手データを"
                    "取得できませんでした。"
                )

            st.session_state.rider_data = rider_data
            st.session_state.rider_logs = rider_logs
            st.session_state.automatic_scores = (
                build_automatic_scores(
                    rider_data
                )
            )

            rider_numbers_for_lineup = sorted(
                int(item["車番"])
                for item in rider_data
                if item.get("車番") is not None
            )

            dom_lineup_groups, dom_lineup_logs = (
                fetch_lineup_browser(
                    selected_race["url"],
                    rider_numbers_for_lineup,
                )
            )

            comment_lineup_groups, comment_lineup_logs = (
                infer_lineup_from_comments(
                    rider_data
                )
            )

            merged_lineup_groups, merge_logs = (
                merge_dom_order_with_comment_groups(
                    dom_lineup_groups,
                    comment_lineup_groups,
                    rider_numbers_for_lineup,
                )
            )

            (
                lineup_groups,
                lineup_metadata,
                selection_logs,
            ) = select_authoritative_lineup(
                dom_lineup_groups,
                comment_lineup_groups,
                rider_numbers_for_lineup,
            )
            rider_data = [
                {
                    **rider,
                    **lineup_metadata,
                }
                for rider in rider_data
            ]
            st.session_state.rider_data = (
                rider_data
            )
            adopted_numbers = [
                int(number)
                for group in lineup_groups
                for number in group
            ]

            if (
                len(adopted_numbers)
                != len(set(adopted_numbers))
                or set(adopted_numbers)
                != set(rider_numbers_for_lineup)
            ):
                raise RuntimeError(
                    "採用並びが出走車番と"
                    "一致しませんでした。"
                )

            adopted_lineup_text = " / ".join(
                "-".join(
                    str(number)
                    for number in group
                )
                for group in lineup_groups
            )
            reference_lineup_text = " / ".join(
                "-".join(
                    str(number)
                    for number in group
                )
                for group in merged_lineup_groups
            )
            lineup_logs = (
                dom_lineup_logs
                + comment_lineup_logs
                + merge_logs
                + selection_logs
                + [
                    (
                        "DOM統合参考結果: "
                        f"{reference_lineup_text}"
                    ),
                    (
                        "採用並び（保存・予測に使用）: "
                        f"{adopted_lineup_text}"
                    ),
                ]
            )

            st.session_state.lineup_groups = lineup_groups
            st.session_state.lineup_logs = lineup_logs

            save_current_race_snapshot(
                odds_rows=st.session_state.odds,
                riders=rider_data,
                lineup_groups=lineup_groups,
                odds_logs=(
                    st.session_state.odds_logs
                ),
                race_date=selected_date,
                venue=venue,
                race_number=int(
                    selected_race[
                        "race_number"
                    ]
                ),
                race_url=str(
                    selected_race["url"]
                ),
                race_title=(
                    f"{venue} {selected_label}"
                ),
            )

            if st.session_state.odds:
                st.session_state[
                    "current_collection_scope"
                ] = "full"
                st.session_state[
                    "current_collection_message"
                ] = (
                    f"選手{len(rider_data)}人・並び・"
                    "完全オッズを取得しました。"
                )
            else:
                st.session_state[
                    "current_collection_scope"
                ] = "independent"
                st.session_state[
                    "current_collection_message"
                ] = (
                    f"選手{len(rider_data)}人と並びを"
                    "取得しました。オッズ非表示のため、"
                    "オッズ非依存AI用として保存しました。"
                )

        except Exception as exc:
            st.session_state[
                "current_collection_scope"
            ] = "error"
            st.session_state[
                "current_collection_message"
            ] = (
                "出走表・並び取得エラー: "
                f"{type(exc).__name__}: {exc}"
            )

    loaded_race_url = str(
        st.session_state.get(
            "selected_race_url",
            "",
        )
    )
    selected_race_is_loaded = (
        loaded_race_url
        == str(selected_race["url"])
    )
    collection_scope = (
        st.session_state.get(
            "current_collection_scope",
            "",
        )
        if selected_race_is_loaded
        else ""
    )
    collection_message = (
        st.session_state.get(
            "current_collection_message",
            "",
        )
        if selected_race_is_loaded
        else ""
    )

    if (
        collection_scope == "independent"
        and collection_message
    ):
        st.info(collection_message)
    elif (
        collection_scope == "full"
        and collection_message
    ):
        st.success(collection_message)
    elif (
        collection_scope == "error"
        and collection_message
    ):
        st.error(collection_message)


odds = st.session_state.odds

if odds:
    st.divider()
    st.subheader("取得オッズ")

    st.success(
        f"3連単オッズを{len(odds)}件取得しました。"
    )

    render_odds_table(odds)

    st.download_button(
        "オッズCSVをダウンロード",
        data=make_csv(
            odds,
            ["人気", "組番", "オッズ"],
        ),
        file_name=(
            f"{selected_date:%Y%m%d}_"
            f"{venue}_{selected_label}_"
            "trifecta_odds.csv"
        ),
        mime="text/csv",
    )

    st.divider()
    st.subheader("展開指数")

    current_lineup_groups = (
        st.session_state.get(
            "lineup_groups",
            [],
        )
    )

    rider_indices_for_display = (
        calculate_rider_indices(
            st.session_state.rider_data,
            current_lineup_groups,
        )
    )

    if rider_indices_for_display:
        index_rows = []

        for number in sorted(
            rider_indices_for_display
        ):
            item = rider_indices_for_display[
                number
            ]

            position = int(
                item.get(
                    "ライン位置",
                    0,
                )
            )

            if int(
                item.get(
                    "ライン長",
                    1,
                )
            ) <= 1:
                position_name = "単騎"
            elif position == 0:
                position_name = "先頭"
            elif position == 1:
                position_name = "番手"
            elif position == 2:
                position_name = "3番手"
            else:
                position_name = (
                    f"{position + 1}番手"
                )

            index_rows.append(
                {
                    "車番": number,
                    "選手名": item["選手名"],
                    "AI印": item["AI印"],
                    "ライン": item["ライン"],
                    "位置": position_name,
                    "競走得点順位": (
                        item["競走得点順位"]
                    ),
                    "先行指数": item["先行指数"],
                    "捲り指数": item["捲り指数"],
                    "追込指数": item["追込指数"],
                }
            )

        index_table_rows = []

        for item in index_rows:
            index_table_rows.append(
                f"""
                <tr>
                    <td>{item["車番"]}</td>
                    <td>{escape(str(item["選手名"]))}</td>
                    <td>{escape(str(item["AI印"]))}</td>
                    <td>{escape(str(item["ライン"]))}</td>
                    <td>{escape(str(item["位置"]))}</td>
                    <td>{item["競走得点順位"]}</td>
                    <td>{float(item["先行指数"]):.1f}</td>
                    <td>{float(item["捲り指数"]):.1f}</td>
                    <td>{float(item["追込指数"]):.1f}</td>
                </tr>
                """
            )

        index_table_html = f"""
        <div style="
            max-height:480px;
            overflow:auto;
            border:1px solid #ddd;
            border-radius:4px;
        ">
            <table style="
                width:100%;
                border-collapse:collapse;
                font-size:14px;
            ">
                <thead>
                    <tr style="
                        position:sticky;
                        top:0;
                        background:#f5f5f5;
                    ">
                        <th style="padding:8px;">車番</th>
                        <th style="padding:8px;">選手名</th>
                        <th style="padding:8px;">AI印</th>
                        <th style="padding:8px;">ライン</th>
                        <th style="padding:8px;">位置</th>
                        <th style="padding:8px;">競走得点順位</th>
                        <th style="padding:8px;">先行指数</th>
                        <th style="padding:8px;">捲り指数</th>
                        <th style="padding:8px;">追込指数</th>
                    </tr>
                </thead>
                <tbody>
                    {''.join(index_table_rows)}
                </tbody>
            </table>
        </div>
        """

        st.markdown(
            index_table_html,
            unsafe_allow_html=True,
        )

    st.divider()
# 確率モデルは専用フォームで確定する。
# 選択操作だけではStreamlit全体を再実行しない。
if "committed_model_mode" not in st.session_state:
    st.session_state.committed_model_mode = "バランス型"

model_mode_options = [
    "的中率重視",
    "バランス型",
    "回収率重視",
]

current_model_index = model_mode_options.index(
    st.session_state.committed_model_mode
)

with st.form(
    "probability_model_selection_form",
    clear_on_submit=False,
    enter_to_submit=False,
):
    pending_model_mode = st.selectbox(
        "確率モデルモード",
        options=model_mode_options,
        index=current_model_index,
        help=(
            "選択しただけでは再計算されません。"
            "下の反映ボタンを押した時だけ設定を確定します。"
        ),
        key="pending_model_mode",
    )

    apply_model_mode = st.form_submit_button(
        "確率モデル設定を反映"
    )

if apply_model_mode:
    st.session_state.committed_model_mode = (
        pending_model_mode
    )

    # 前回の計算結果を消し、異なるモデルの結果が
    # 混在しないようにする。
    st.session_state.expected_values = []
    st.session_state.bet_plan = []
    st.session_state.bet_plan_executed = False
    st.session_state.bet_plan_message = ""
    st.session_state.bet_plan_candidate_count = 0
    st.session_state.calculation_logs = []

    st.success(
        "確率モデルを"
        f"「{pending_model_mode}」へ変更しました。"
        "次に一括実行ボタンを押してください。"
    )

model_mode = st.session_state.committed_model_mode

st.caption(
    f"現在適用中の確率モデル：{model_mode}"
)

with st.form(
    "ev_settings_form",
    clear_on_submit=False,
):

    # ページ移動後も取得データを利用できるよう、
    # 現在レースをJSONへ保存する。
    snapshot_odds = (
        st.session_state.get("odds", [])
        or st.session_state.get(
            "odds_rows",
            [],
        )
    )

    snapshot_riders = (
        st.session_state.get(
            "rider_data",
            [],
        )
    )

    snapshot_lineup = (
        st.session_state.get(
            "lineup_groups",
            [],
        )
    )

    snapshot_odds_logs = (
        st.session_state.get(
            "odds_logs",
            [],
        )
    )

    if snapshot_riders:
        try:
            save_current_race_snapshot(
                odds_rows=snapshot_odds,
                riders=snapshot_riders,
                lineup_groups=snapshot_lineup,
                odds_logs=snapshot_odds_logs,
                race_date=st.session_state.get(
                    "selected_date",
                    "",
                ),
                venue=st.session_state.get(
                    "venue",
                    st.session_state.get(
                        "selected_venue",
                        "",
                    ),
                ),
                race_number=st.session_state.get(
                    "race_number",
                    st.session_state.get(
                        "selected_race_number",
                        0,
                    ),
                ),
                race_url=st.session_state.get(
                    "racecard_url",
                    st.session_state.get(
                        "selected_race_url",
                        "",
                    ),
                ),
                race_title=st.session_state.get(
                    "race_title",
                    "",
                ),
            )
        except Exception as snapshot_error:
            st.session_state[
                "snapshot_save_error"
            ] = str(snapshot_error)


    st.subheader("選手評価と期待値設定")

    probability_col1, probability_col2 = st.columns(2)

    with probability_col1:
        market_blend = st.slider(
            "市場確率の混合率",
            min_value=0.0,
            max_value=0.90,
            value=float(
                st.session_state.get(
                    "market_blend_value",
                    0.55,
                )
            ),
            step=0.05,
            key="market_blend_value",
            help=(
                "高いほど市場オッズに近づき、"
                "極端な大穴評価を抑えます。"
            ),
        )

    with probability_col2:
        maximum_expected_return = st.number_input(
            "期待回収率の計算上限",
            min_value=1.0,
            max_value=10.0,
            value=float(
                st.session_state.get(
                    "maximum_expected_return_value",
                    3.0,
                )
            ),
            step=0.25,
            key="maximum_expected_return_value",
        )

    riders = extract_rider_numbers(odds)

    st.write(
        "各選手を100点満点の目安で評価してください。"
        "絶対値より、選手間の点差が重要です。"
    )

    scores: dict[int, float] = {}

    if not riders:
        if st.session_state.rider_data:
            st.info(
                "オッズが非表示のため、"
                "期待値計算は利用できません。"
                "「オッズ非依存AI」ページでは"
                "取得済みの選手・並びを使えます。"
            )
        else:
            st.warning(
                "選手データがありません。"
                "先にメイン画面で"
                "「レースデータを取得」を"
                "押してください。"
            )

        score_columns = []
    else:
        score_columns = st.columns(
            max(
                1,
                min(len(riders), 9),
            )
        )

    for index, rider in enumerate(riders):
        with score_columns[index % len(score_columns)]:
            rider_information = next(
                (
                    item
                    for item in st.session_state.rider_data
                    if int(item.get("車番", 0)) == rider
                ),
                {},
            )

            rider_name = rider_information.get(
                "選手名",
                "",
            )

            automatic_value = float(
                st.session_state.automatic_scores.get(
                    rider,
                    50.0,
                )
            )

            label = (
                f"{rider}番車 {rider_name}"
                if rider_name
                else f"{rider}番車"
            )

            scores[rider] = st.number_input(
                label,
                min_value=0.0,
                max_value=100.0,
                value=automatic_value,
                step=0.5,
                key=f"score_{rider}",
            )

            details = []

            for key in (
                "AI印",
                "競走得点",
                "脚質",
                "B",
                "H",
                "S",
            ):
                value = rider_information.get(key)

                if value not in (None, ""):
                    details.append(
                        f"{key}:{value}"
                    )

            if details:
                st.caption(
                    " / ".join(details)
                )

    settings_left, settings_middle, settings_right = (
        st.columns(3)
    )

    with settings_left:
        temperature = st.number_input(
            "確率の平準化",
            min_value=0.1,
            max_value=20.0,
            value=5.0,
            step=0.1,
            help=(
                "小さいほど高評価選手を強く信頼し、"
                "大きいほど確率差を小さくします。"
            ),
        )

    with settings_middle:
        minimum_expected_return = st.number_input(
            "最低期待回収率",
            min_value=0.5,
            max_value=5.0,
            value=1.10,
            step=0.05,
        )

    with settings_right:
        bankroll = st.number_input(
            "参考資金",
            min_value=1_000,
            max_value=10_000_000,
            value=int(
                st.session_state.get(
                    "plan_budget",
                    10_000,
                )
            ),
            step=1_000,
            key="plan_budget",
        )

    st.markdown("#### 着順別ライン確率補正")

    use_sequence_adjustment = st.checkbox(
        "3連単の着順へライン補正を使用する",
        value=True,
        help=(
            "同ラインの先頭・番手・3番手が"
            "連続して入着する組番を相対的に加点します。"
        ),
    )

    sequence_col1, sequence_col2 = st.columns(2)
    sequence_col3, sequence_col4 = st.columns(2)

    with sequence_col1:
        line_exact_bonus = st.number_input(
            "ライン順完全一致倍率",
            min_value=0.50,
            max_value=3.00,
            value=1.30,
            step=0.05,
            help="例：7-4-1ラインで7-4-1決着",
        )

    with sequence_col2:
        line_pair_bonus = st.number_input(
            "先頭・番手連携倍率",
            min_value=0.50,
            max_value=2.00,
            value=1.12,
            step=0.02,
        )

    with sequence_col3:
        second_over_leader_bonus = st.number_input(
            "番手差し倍率",
            min_value=0.50,
            max_value=2.00,
            value=1.15,
            step=0.05,
            help="例：7-4ラインで4-7決着",
        )

    with sequence_col4:
        second_third_bonus = st.number_input(
            "番手・3番手連携倍率",
            min_value=0.50,
            max_value=2.00,
            value=1.08,
            step=0.02,
        )

    kelly_fraction = st.slider(
        "ケリー基準の使用割合",
        min_value=0.0,
        max_value=1.0,
        value=0.25,
        step=0.05,
        help=(
            "1.0はフルケリー、0.25は4分の1ケリーです。"
        ),
    )

    if st.form_submit_button(
        "妙味期待値を計算",
        type="primary",
    ):
        lineup_groups = st.session_state.get(
            "lineup_groups",
            [],
        )

        results = calculate_expected_values_v2(
            odds,
            scores=scores,
            riders=st.session_state.get(
                "rider_data",
                [],
            ),
            model_mode=st.session_state.get(
                "committed_model_mode",
                "バランス型",
            ),
            market_blend=float(
                st.session_state.get(
                    "market_blend_value",
                    0.55,
                )
            ),
            maximum_expected_return=float(
                st.session_state.get(
                    "maximum_expected_return_value",
                    3.0,
                )
            ),
            temperature=temperature,
            bankroll=int(bankroll),
            kelly_fraction=kelly_fraction,
            lineup_groups=lineup_groups,
            use_sequence_adjustment=(
                use_sequence_adjustment
            ),
            line_exact_bonus=line_exact_bonus,
            line_pair_bonus=line_pair_bonus,
            second_over_leader_bonus=(
                second_over_leader_bonus
            ),
            second_third_bonus=(
                second_third_bonus
            ),
        )

        rider_indices = calculate_rider_indices(
            st.session_state.rider_data,
            lineup_groups,
        )

        results = build_bet_reasons(
            results,
            rider_indices,
            lineup_groups,
        )

        st.session_state.expected_values = results
        st.session_state.bet_plan = []
        st.session_state.bet_plan_executed = False
        st.session_state.bet_plan_message = ""
        st.session_state.bet_plan_candidate_count = 0


expected_values = st.session_state.expected_values

if expected_values:
    st.divider()
    st.subheader("妙味期待値ランキング")

    filtered_results = [
        row
        for row in expected_values
        if row["期待回収率"]
        >= minimum_expected_return
    ]

    display_count = st.slider(
        "表示件数",
        min_value=5,
        max_value=min(
            100,
            len(expected_values),
        ),
        value=min(
            30,
            len(expected_values),
        ),
        step=5,
    )

    if filtered_results:
        st.success(
            f"最低期待回収率"
            f"{minimum_expected_return:.2f}以上："
            f"{len(filtered_results)}件"
        )
    else:
        st.warning(
            "設定した最低期待回収率を超える"
            "買い目はありません。"
        )

    render_expected_value_table(
        expected_values[:display_count],
        threshold=minimum_expected_return,
    )

    export_rows = []

    for row in expected_values:
        export_rows.append(
            {
                "人気": row["人気"],
                "組番": row["組番"],
                "オッズ": round(row["オッズ"], 3),
                "モデル確率": round(
                    row["モデル確率"],
                    8,
                ),
                "市場確率": round(
                    row["市場確率"],
                    8,
                ),
                "フェアオッズ": round(
                    row["フェアオッズ"],
                    3,
                ),
                "期待回収率": round(
                    row["期待回収率"],
                    4,
                ),
                "期待利益率": round(
                    row["期待利益率"],
                    4,
                ),
                "妙味倍率": round(
                    row["妙味倍率"],
                    4,
                ),
                "参考購入額": row["参考購入額"],
            }
        )

    st.divider()
    st.subheader("予算別・買い目自動配分")

    allocation_col1, allocation_col2 = st.columns(2)
    allocation_col3, allocation_col4 = st.columns(2)

    with allocation_col1:
        betting_budget = st.number_input(
            "今回の購入予算",
            min_value=100,
            max_value=10_000_000,
            value=10_000,
            step=1_000,
        )

    with allocation_col2:
        maximum_bets = st.number_input(
            "最大買い目数",
            min_value=1,
            max_value=50,
            value=int(
                st.session_state.get(
                    "plan_maximum_bets",
                    10,
                )
            ),
            step=1,
            key="plan_maximum_bets",
        )

    with allocation_col3:
        maximum_bet_ratio = st.slider(
            "1点あたり最大予算比率",
            min_value=0.05,
            max_value=1.00,
            value=float(
                st.session_state.get(
                    "plan_maximum_ratio",
                    0.25,
                )
            ),
            step=0.05,
            key="plan_maximum_ratio",
        )

    filter_col1, filter_col2, filter_col3 = (
        st.columns(3)
    )

    with filter_col1:
        maximum_odds = st.number_input(
            "最大対象オッズ",
            min_value=10.0,
            max_value=9999.9,
            value=300.0,
            step=10.0,
        )

    with filter_col2:
        maximum_popularity = st.number_input(
            "最大対象人気順位",
            min_value=1,
            max_value=504,
            value=120,
            step=10,
        )

    with filter_col3:
        minimum_model_probability = st.number_input(
            "最低モデル確率",
            min_value=0.0001,
            max_value=0.1000,
            value=0.0010,
            step=0.0005,
            format="%.4f",
        )

    with allocation_col4:
        minimum_unit = st.selectbox(
            "購入単位",
            options=[100, 200, 500, 1000],
            index=0,
            format_func=lambda value: f"{value:,}円",
        )

    qualifying_rows = [
        row
        for row in expected_values
        if float(row.get("期待回収率", 0))
        >= float(minimum_expected_return)
    ]

    positive_kelly_rows = [
        row
        for row in qualifying_rows
        if float(row.get("モデル確率", 0))
        * float(row.get("オッズ", 0)) > 1.0
    ]

    diagnostic_col1, diagnostic_col2 = st.columns(2)

    with diagnostic_col1:
        st.metric(
            "最低期待回収率以上",
            f"{len(qualifying_rows)}件",
        )

    with diagnostic_col2:
        st.metric(
            "配分候補",
            f"{len(positive_kelly_rows)}件",
        )

    if st.button(
        "期待値計算から買い目作成まで一括実行",
        type="primary",
    ):
        lineup_groups_for_plan = (
            st.session_state.get(
                "lineup_groups",
                [],
            )
        )

        calculated_results = (
            calculate_expected_values_v2(
                odds,
                scores=scores,
                temperature=temperature,
                bankroll=int(bankroll),
                kelly_fraction=kelly_fraction,
                lineup_groups=(
                    lineup_groups_for_plan
                ),
                use_sequence_adjustment=(
                    use_sequence_adjustment
                ),
                line_exact_bonus=(
                    line_exact_bonus
                ),
                line_pair_bonus=(
                    line_pair_bonus
                ),
                second_over_leader_bonus=(
                    second_over_leader_bonus
                ),
                second_third_bonus=(
                    second_third_bonus
                ),
            )
        )

        rider_indices = calculate_rider_indices(
            st.session_state.rider_data,
            lineup_groups_for_plan,
        )

        calculated_results = build_bet_reasons(
            calculated_results,
            rider_indices,
            lineup_groups_for_plan,
        )

        st.session_state.expected_values = (
            calculated_results
        )

        generated_plan = allocate_bet_budget(
            calculated_results,
            budget=int(betting_budget),
            minimum_expected_return=float(
                minimum_expected_return
            ),
            maximum_bets=int(maximum_bets),
            minimum_unit=int(minimum_unit),
            maximum_bet_ratio=float(
                maximum_bet_ratio
            ),
            maximum_odds=float(
                maximum_odds
            ),
            maximum_popularity=int(
                maximum_popularity
            ),
            minimum_model_probability=float(
                minimum_model_probability
            ),
        )

        st.session_state.bet_plan = generated_plan
        st.session_state.bet_plan_executed = True
        st.session_state.bet_plan_candidate_count = len(
            generated_plan
        )

        if generated_plan:
            st.session_state.bet_plan_message = (
                f"{len(generated_plan)}点の"
                "買い目プランを作成しました。"
            )
            st.success(
                st.session_state.bet_plan_message
            )
            st.rerun()
        else:
            st.session_state.bet_plan_message = (
                "現在の設定では購入対象が"
                "見つかりませんでした。"
            )
            st.warning(
                st.session_state.bet_plan_message
            )
            st.rerun()

    if st.button(
        "予算から買い目を自動配分",
    ):
        generated_plan = allocate_bet_budget(
            expected_values,
            budget=int(betting_budget),
            minimum_expected_return=(
                minimum_expected_return
            ),
            maximum_bets=int(maximum_bets),
            minimum_unit=int(minimum_unit),
            maximum_bet_ratio=(
                maximum_bet_ratio
            ),
        )

        st.session_state.bet_plan = generated_plan
        st.session_state.bet_plan_executed = True
        st.session_state.bet_plan_candidate_count = len(
            positive_kelly_rows
        )

        if not generated_plan:
            if not qualifying_rows:
                st.session_state.bet_plan_message = (
                    "買い目候補は0件です。"
                    "最低期待回収率を満たす組番がありません。"
                )
            elif not positive_kelly_rows:
                st.session_state.bet_plan_message = (
                    "買い目候補は0件です。"
                    "期待回収率の条件を満たしていても、"
                    "ケリー基準がプラスの組番がありません。"
                )
            else:
                st.session_state.bet_plan_message = (
                    "買い目候補は0件です。"
                    "購入単位、予算、1点上限の設定により"
                    "配分できませんでした。"
                )
        else:
            st.session_state.bet_plan_message = (
                f"買い目候補は{len(generated_plan)}件です。"
            )

        st.rerun()

    bet_plan = st.session_state.get(
        "bet_plan",
        [],
    )

    if bet_plan:
        st.success(
            st.session_state.get(
                "bet_plan_message",
                f"{len(bet_plan)}点の買い目プランを表示します。",
            )
        )

        st.markdown("### 作成された買い目プラン")

        total_purchase = sum(
            int(row["購入額"])
            for row in bet_plan
        )

        total_expected_return = sum(
            float(row["期待払戻"])
            for row in bet_plan
        )

        total_expected_profit = sum(
            float(row["期待利益"])
            for row in bet_plan
        )

        summary_col1, summary_col2, summary_col3 = (
            st.columns(3)
        )

        with summary_col1:
            st.metric(
                "合計購入額",
                f"{total_purchase:,}円",
            )

        with summary_col2:
            st.metric(
                "期待払戻額",
                f"{total_expected_return:,.0f}円",
            )

        with summary_col3:
            st.metric(
                "期待利益",
                f"{total_expected_profit:+,.0f}円",
            )

        plan_table_rows = []

        for index, row in enumerate(
            bet_plan,
            start=1,
        ):
            plan_table_rows.append(
                {
                    "順位": index,
                    "組番": str(row["組番"]),
                    "人気": int(row["人気"]),
                    "オッズ": round(
                        float(row["オッズ"]),
                        1,
                    ),
                    "モデル確率": (
                        f"{float(row['モデル確率']) * 100:.3f}%"
                    ),
                    "期待回収率": round(
                        float(row["期待回収率"]),
                        3,
                    ),
                    "購入額": (
                        f"{int(row['購入額']):,}円"
                    ),
                    "取得時概算払戻": (
                        f"{int(row['取得時概算払戻']):,}円"
                    ),
                    "的中時利益": (
                        f"{int(row['的中時利益']):+,}円"
                    ),
                    "期待利益": (
                        f"{float(row['期待利益']):+,.0f}円"
                    ),
                    "予算比率": (
                        f"{float(row['予算比率']) * 100:.1f}%"
                    ),
                    "購入理由": str(
                        row.get(
                            "購入理由",
                            "",
                        )
                    ),
                }
            )

        st.dataframe(
            plan_table_rows,
            use_container_width=True,
            hide_index=True,
        )

        bet_export_rows = []

        for row in bet_plan:
            bet_export_rows.append(
                {
                    "組番": row["組番"],
                    "人気": row["人気"],
                    "オッズ": row["オッズ"],
                    "モデル確率": round(
                        row["モデル確率"],
                        8,
                    ),
                    "期待回収率": round(
                        row["期待回収率"],
                        4,
                    ),
                    "購入額": row["購入額"],
                    "取得時概算払戻": row["取得時概算払戻"],
                    "的中時利益": row["的中時利益"],
                    "期待払戻": row["期待払戻"],
                    "期待利益": row["期待利益"],
                    "予算比率": round(
                        row["予算比率"],
                        4,
                    ),
                    "購入理由": row.get(
                        "購入理由",
                        "",
                    ),
                }
            )

        st.warning(
            "概算払戻はオッズ取得時点の数値です。"
            "投票締切までのオッズ変動や同着により、"
            "実際の確定払戻額とは異なる場合があります。"
        )

        st.download_button(
            "買い目プランCSVをダウンロード",
            data=make_csv(
                bet_export_rows,
                [
                    "組番",
                    "人気",
                    "オッズ",
                    "モデル確率",
                    "期待回収率",
                    "購入額",
                    "取得時概算払戻",
                    "的中時利益",
                    "期待払戻",
                    "期待利益",
                    "予算比率",
                    "購入理由",
                ],
            ),
            file_name=(
                f"{selected_date:%Y%m%d}_"
                f"{venue}_{selected_label}_"
                "bet_plan.csv"
            ),
            mime="text/csv",
        )

    else:
        plan_executed = st.session_state.get(
            "bet_plan_executed",
            False,
        )

        plan_message = st.session_state.get(
            "bet_plan_message",
            "",
        )

        if plan_executed:
            st.error(
                plan_message
                or "買い目候補は0件です。"
            )

            st.markdown("### 買い目プラン：0件")

            reason_rows = []

            if len(qualifying_rows) == 0:
                reason_rows.append(
                    "最低期待回収率以上の組番が0件"
                )

            if len(positive_kelly_rows) == 0:
                reason_rows.append(
                    "ケリー基準がプラスの組番が0件"
                )

            if int(betting_budget) < int(minimum_unit):
                reason_rows.append(
                    "購入予算が購入単位を下回っています"
                )

            if reason_rows:
                st.write("作成できなかった理由：")

                for reason in reason_rows:
                    st.write(f"・{reason}")

            st.info(
                "動作確認では最低期待回収率を1.00、"
                "購入予算を10,000円、購入単位を100円に"
                "設定してください。"
            )

        else:
            st.info(
                "まだ買い目プランを作成していません。"
                "設定後に「予算から買い目を自動配分」を"
                "押してください。"
            )

    st.download_button(
        "期待値CSVをダウンロード",
        data=make_csv(
            export_rows,
            [
                "人気",
                "組番",
                "オッズ",
                "モデル確率",
                "市場確率",
                "フェアオッズ",
                "期待回収率",
                "期待利益率",
                "妙味倍率",
                "参考購入額",
            ],
        ),
        file_name=(
            f"{selected_date:%Y%m%d}_"
            f"{venue}_{selected_label}_"
            "expected_values.csv"
        ),
        mime="text/csv",
    )

    with st.expander("期待値の計算方法"):
        st.write(
            "期待回収率 ＝ モデル的中確率 × オッズ"
        )
        st.write(
            "妙味倍率 ＝ モデル的中確率 ÷ 市場確率"
        )
        st.write(
            "フェアオッズ ＝ 1 ÷ モデル的中確率"
        )
        st.warning(
            "評価点と確率モデルに基づく推定値であり、"
            "利益や的中を保証するものではありません。"
        )


with st.expander("取得ログ・自己診断"):
    logs = (
        st.session_state.catalog_logs
        + st.session_state.odds_logs
        + st.session_state.rider_logs
        + st.session_state.lineup_logs
    )

    st.code(
        "\n".join(logs)
        if logs
        else "まだ取得を実行していません。",
        language=None,
    )
